...
function getDSECourses(summer=true; filenameprefix=DEFAULT_FILENAME_PREFIX, data_table=DEFAULT_DATA_TABLE,
    ...
    # # copy degree plan
    copied_dp = db_file # copyDegreePlanSpreadSheet(; filenameprefix)
...
function draw_summer_chart(filename::Union{String,Nothing}, is_summer::Bool=true, term_field::Symbol=:Term;
    db_file=DEFAULT_FILENAME_LOCATION,
    data_table="", filenameprefix="", degree_name="")
    clrs = theme_colors()
    ...
    df = getDSECourses(is_summer; data_table=data_table, filenameprefix=filenameprefix, db_file=db_file)
