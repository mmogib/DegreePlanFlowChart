...
function draw_summer_chart(filename::Union{String, Nothing}, is_summer::Bool = true, term_field::Symbol = :Term; data_table = "", filenameprefix = "", degree_name = "")
	...
	for course in all_courses
		...
		if course.code in ["DATA 399", "DATA 398"]
			sethue("black")
			pt_summer = is_summer ? Point(pt.x, pt.y + 150) : Point(pt.x, pt.y + 220)
			...
			arrow(Point(pt_summer.x, pt_summer.y - 5 - 0.5course.w), Point(course.x, course.y + course.h / 2))
			fontsize(14)
		end
	end
	finish()
...
draw_summer_chart(output = :pdf, is_summer = true,
	filenameprefix = "Yazan_DegreePlan",
	data_table = "Data",
	degree_name = "BS-X in Internet of Things & Data Analytics",
)
